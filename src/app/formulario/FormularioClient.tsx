...
import { Check, AlertCircle, User, Mail, Calendar, MapPin, Loader2, Info, Clock } from 'lucide-react'
...
export default function RegistroAsistenciaPage() {
  ...
  const [cargandoReunion, setCargandoReunion] = useState(true)
  ...
  const fetchReunion = async (reunionId: number) => {
    ...
    try {
      ...
      setCargandoReunion(false)
    }
  }

  useEffect(() => {
    if (reunionId) fetchReunion(reunionId)
  }, [reunionId])
  ...
  const formatearHora = (hora: string) =>
    new Date(`2000-01-01T${hora}`).toLocaleTimeString('es-ES', {
      hour: '2-digit',
      minute: '2-digit'
    })
  ...
  const handleReset = () => {
    setFormData({ documento: '', correo: '', telefono: '' })
    setErrors({})
    setMensaje(null)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#8B5A3C] to-[#6D4428] flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-lg p-6 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-4">No se pudo cargar la información de la reunión solicitada.</p>
        </div>
      </div>
    )
  }
          ...
          <div className="p-6">
            <div className="bg-gray-50 rounded-lg p-4 mb-6 space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-800 truncate">{datosConfirmados.correo}</span>
              </div>
              <div className="flex justify-between">
        ...
        <div className="bg-white rounded-b-xl shadow-lg p-6 -mt-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-[#8B5A3C] text-white rounded-lg hover:bg-[#6D4428] disabled:opacity-70 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
